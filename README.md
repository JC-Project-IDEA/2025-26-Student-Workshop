...
//Variables for Theremin sense auto-calibration
  ...
  void setOffset() {
    Offset = pulseWidth*1.0001; //<<<<<<<<<<<<< Auto
    //Offset = 3008.3;  //<<<<<<<<<<<<< Manual
    OffsetFlag = 1;
  }
...
//For all sensors
  float smoothingFactor = 0.01; //<<<<<<<<<<<<< Sensor value smoothness
...
//MIDI message - Touch Sensor
  ...
  int TO_midiNotes[4] = {43, 47, 50, 54};
...
//---------------------------------------------- Theremin reading
  if (newPulse) {
    ...
    CapSenseValue = smoothedCapSense*pulseScale;
    if(CapSenseValue < 0 ){
      CapSenseValue = 0;
    }
    if(CapSenseValue > 127 ){
      CapSenseValue = 127;
    }
  }

//---------------------------------------------- Theremin MIDI output
    TH_midiCCValue = CapSenseValue;
...
//---------------------------------------------- IRSensor MIDI output
  IR_midiCCValue = (127-(Distance_cm-10)*3.628);
...
//---------------------------------------------- Serial Print for debug
  if(TimeNow - TimeSerial >= 300){
    Serial.print(millis());        // check on performance in milliseconds
    Serial.print("\t");                    // tab character for debug windown spacing
    Serial.print("Offset: ");
    Serial.print(Offset);                  // print theremin sensor output
    Serial.print("\t");                    // tab character for debug windown spacing
    Serial.print("Theremin: ");
    Serial.print(pulseWidth_cal);                  // print theremin sensor output
    Serial.print("\t");
    Serial.print("IR: ");
    Serial.print(smoothedDistance);                  // print IR sensor output
    Serial.print("\t");
